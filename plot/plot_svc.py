...
def draw_a_wax_wane(ax, start, stop, length, y_base, bar_offset, bar_mult, label, colorstring, artifact_flags, time_range):
    
#      print("in draw_wax_wane")
      ax.text(start-400, y_base, label, color=colorstring)
      for j in range(0, length-16, 16):        
         ybot = y_base - artifact_flags[time_range[j:j+16]]/2
         ytop = y_base + artifact_flags[time_range[j:j+16]]/2
      ...
      rectangle = plt.Rectangle((stop+bar_offset, y_base), total*bar_mult/2, 20, color=colorstring)
...
def draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, label, colorstring, artifact_flags, time_range, rmsvalue):
    
#      print("in draw_wax_wane")
      ax.text(start-400, y_base, label, color=colorstring)
      for j in range(0, length-16, 16):        
         ybot = y_base - artifact_flags[time_range[j:j+16]]/20
         ytop = y_base + artifact_flags[time_range[j:j+16]]/20
         plt.fill_between(time_range[j:j+16], ytop, ybot, where=(ytop >= ybot), color=colorstring)
...
#  CALCULATE THE POWER FOR THIS CHANNEL AND SHOW IT
        ...
        rectangle = plt.Rectangle((stop+3*square_offset+5, y_position), stand*bar_mult, square_offset-1, color=colorlist[0])
      
        ax.add_patch(rectangle)
      
        bar_mult_powers = 1.5
        lodelt = np.sum(lodelta_artifact[i,time_range]/1000)
        rectangle = plt.Rectangle((stop+6*square_offset, y_position), square_offset, lodelt*bar_mult, color=colorlist[1])
        ax.add_patch(rectangle)
        rectangle = plt.Rectangle((stop+7*square_offset, y_position), square_offset, ((np.sum(delta_artifact[i, time_range]))/100)*bar_mult_powers, color=colorlist[2])
        ax.add_patch(rectangle)
        rectangle = plt.Rectangle((stop+8*square_offset, y_position),square_offset, ((np.sum(theta_artifact[i, time_range]))/100)*bar_mult_powers, color=colorlist[3])
        ax.add_patch(rectangle)        
        rectangle = plt.Rectangle((stop+9*square_offset, y_position), square_offset, ((np.sum(loalpha_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[4])
        ax.add_patch(rectangle)        
        rectangle = plt.Rectangle((stop+10*square_offset, y_position), square_offset, ((np.sum(alpha_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[5])
        ax.add_patch(rectangle)        
        rectangle = plt.Rectangle((stop+11*square_offset, y_position), square_offset, ((np.sum(hialpha_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[6])
        ax.add_patch(rectangle)        
        rectangle = plt.Rectangle((stop+12*square_offset, y_position), square_offset, ((np.sum(lobeta_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[7])
        ax.add_patch(rectangle)        
        rectangle = plt.Rectangle((stop+13*square_offset, y_position), square_offset, ((np.sum(beta_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[8])
        ax.add_patch(rectangle)        
        rectangle = plt.Rectangle((stop+14*square_offset, y_position), square_offset, ((np.sum(hibeta_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[9])
        ax.add_patch(rectangle)
        rectangle = plt.Rectangle((stop+15*square_offset, y_position), square_offset, ((np.sum(gamma_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[10])
        ax.add_patch(rectangle)
        rectangle = plt.Rectangle((stop+16*square_offset, y_position), square_offset, ((np.sum(higamma_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[11])
        ax.add_patch(rectangle)
        rectangle = plt.Rectangle((stop+17*square_offset, y_position), square_offset, ((np.sum(f60hz_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[12])
        ax.add_patch(rectangle)
        rectangle = plt.Rectangle((stop+18*square_offset, y_position), square_offset, ((np.sum(f61hz_artifact[i,time_range]))/100)*bar_mult_powers, color=colorlist[13])
        ax.add_patch(rectangle)
...
#  CALCULATE ENTROPY OF THIS CHANNEL FOR THIS PAGE
        ...
        rectangle = plt.Rectangle((stop+23*square_offset+10, y_position), entropy*10*bar_mult, square_offset, color="Black")
        ax.add_patch(rectangle)
        return(y_position)
...
#          tstring = f"{channel_labels_short[i]}:"
          ...
          if peak_indexl != 0:
              ...
              tstring = f"{valuel:.1f}"
              ax.text(start+2950, y_min + 450 - 20 * i, tstring)
...
#      draw_a_wax_wane(ax, start, stop, length, y_base, bar_offset, bar_mult, "LoD:", colorlist[1], lodelta_artifact_tot, time_range)
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "LoD:", colorlist[1], lodelta_artifact_tot, time_range, lodelta_rms_tot/n)
...
# DRAW THE DELTA LINE 3
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "D:", colorlist[2], delta_artifact_tot, time_range, delta_rms_tot/n)
...
# DRAW THE THETA LINE 4
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "T:", colorlist[3], theta_artifact_tot, time_range, theta_rms_tot/n)
...
# DRAW THE ALPHA1 LINE 5
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "A1:", colorlist[4], loalpha_artifact_tot, time_range, loalpha_rms_tot/n)
...
# DRAW THE ALPHA LINE 6
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "A:", colorlist[5], alpha_artifact_tot, time_range, alpha_rms_tot/n)
...
# DRAW THE ALPHA2 LINE 7
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "A2:", colorlist[6], hialpha_artifact_tot, time_range, hialpha_rms_tot/n)
...
# DRAW THE LOBETA LINE 8
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "LoB:", colorlist[7], lobeta_artifact_tot, time_range, lobeta_rms_tot/n)
...
# DRAW THE BETA LINE 9
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "B:", colorlist[8], beta_artifact_tot, time_range, beta_rms_tot/n)
...
# DRAW THE HIBETA LINE 10
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "HiB:", colorlist[9], hibeta_artifact_tot, time_range, hibeta_rms_tot/n)
...
# DRAW THE GAMMA LINE 11
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "G:", colorlist[10], gamma_artifact_tot, time_range, gamma_rms_tot/n)
        for j in  range(1, 13):
...
# DRAW THE HIGAMMA LINE 12
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "HiG:", colorlist[11], higamma_artifact_tot, time_range, higamma_rms_tot/n)
        for j in  range(1, 13):
...
# DRAW THE f60hz LINE 13
        ...
        draw_a_wax_waner(ax, start, stop, length, y_base, bar_offset, bar_mult, "60Hz:", colorlist[12], f60hz_artifact_tot, time_range, f60hz_rms_tot/n)