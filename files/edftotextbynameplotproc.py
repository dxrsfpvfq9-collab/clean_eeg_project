...
# Open the EDF file-------------------------------------------------------------------------------------------
    if selstring[9] != 1 and selstring[8] != 1 and selstring[12] != 1:
      try:
...
#  HERE IS WHERE WE BUILD THE FILTER COEFFICIENTS----------------------------------------------------------------------------------------
    ...
    bla, ala, bhla, ahla = process.tfcfilters.tfcfilterssetup(7.0, 11.0, forder)
    bal, aal, bhal, ahal = process.tfcfilters.tfcfilterssetup(7.0, 13.0, forder)
    bha, aha, bhha, ahha = process.tfcfilters.tfcfilterssetup(9.0, 13.0, forder)
...
#  HERE IS WHERE WE RUN THE ARTIFACT DETECTION ROUTINES ON EACH CHANNEL AND STORE THE FLAGS------------------------------------------------------
    ...
    for chanindex in range (0, n):
      ...
      loalpha_artifact[chanindex,:], loalpha_rms_value = process.detect_artifact.detect_band_with_rms(bla, ala, bhla, ahla, myfilteredsigs[chanindex], numsamples, det_thresh/2)
      alpha_artifact[chanindex,:], alpha_rms_value = process.detect_artifact.detect_band_with_rms(bal, aal, bhal, ahal, myfilteredsigs[chanindex], numsamples, det_thresh/2)
      hialpha_artifact[chanindex,:], hialpha_rms_value = process.detect_artifact.detect_band_with_rms(bha, aha, bhha, ahha, myfilteredsigs[chanindex], numsamples, det_thresh/2)
...
#  THIS IS THE PAGE PROCESSING DOING ONE PAGE AT A TIME BETWEEN START AND STOP TIMES
    pageno = 0
    mymetricsa = np.zeros((int(numpages)+1, 56))
...
#  BEGIN TO COMPILE METRICS INTO METRICS ARRAY FOR EXPORTING TO EXCEL
      name_strings = ["Numsamps", "nchans", "std", "newmax", "std orig", "diff:recon::orig", "diff:clean1::orig", "percent removed",
                      ...
                      "Beta MaxFront", "Front AlphaAsym", "XS Temp.Alpha", "FastAlpha", 'AlphaPeak', 'MidlineBeta', "FocalDelta", "FocalDeltaAmp", 
...
#      mymetricsa = np.zeros(27)
      ...
      mymetricsa[index,22], mymetricsa[index,23], report_strings = process.detect_artifact.detect_phen_fastalpha(loalpha_artifact[:,time_range], alpha_artifact[:,time_range], hialpha_artifact[:,time_range], channel_labels_pre, report_strings, mysigs, numpages)
      mymetricsa[index,24], report_strings = process.detect_artifact.detect_phen_spinbeta(beta_artifact[:,time_range], channel_labels_pre, report_strings)

      mymetricsa[index,25], mymetricsa[index,26], report_strings = process.detect_artifact.detect_focal_delta(delta_artifact[:,time_range], channel_labels_pre, report_strings, sites_rsi)
      mymetricsa[index,27], mymetricsa[index,28], report_strings = process.detect_artifact.detect_focal_theta(theta_artifact[:,time_range], channel_labels_pre, report_strings, sites_rsi)
      mymetricsa[index,29], mymetricsa[index,30], report_strings = process.detect_artifact.detect_focal_hibeta(hibeta_artifact[:,time_range], channel_labels_pre, report_strings, sites_rsi)
      mymetricsa[index,31], mymetricsa[index,32], report_strings = process.detect_artifact.detect_focal_hibeta(beta_artifact[:,time_range], channel_labels_pre, report_strings, sites_rsi) #FOCAL BETA
      mymetricsa[index,33], report_strings = process.detect_artifact.detect_frontal_delta(delta_artifact[:,time_range], channel_labels_pre, report_strings)
      mymetricsa[index,34], report_strings = process.detect_artifact.detect_phen_xsfrontslow(theta_artifact[:,time_range], channel_labels_pre, report_strings) #FRONTAL THETA
      mymetricsa[index,35], report_strings = process.detect_artifact.detect_frontal_delta(gamma_artifact[:,time_range], channel_labels_pre, report_strings) #FRONTAL GAMMA
      mymetricsa[index,36], report_strings = process.detect_artifact.detect_phen_frontasym(gamma_artifact[:,time_range], channel_labels_pre, report_strings) #FRONTAL GAMMA ASYM
      
      mymetricsa[index,37], report_strings = process.detect_artifact.detect_diffuse_delta(delta_artifact[:,time_range], channel_labels_pre, report_strings)
      mymetricsa[index,38], report_strings = process.detect_artifact.detect_diffuse_theta(theta_artifact[:,time_range], channel_labels_pre, report_strings)
      mymetricsa[index,39], report_strings = process.detect_artifact.detect_diffuse_hibeta(hibeta_artifact[:,time_range], channel_labels_pre, report_strings)
      mymetricsa[index,40], report_strings = process.detect_artifact.detect_diffuse_hibeta(beta_artifact[:,time_range], channel_labels_pre, report_strings) #DIFFUSE BETA
      mymetricsa[index,41], report_strings = process.detect_artifact.detect_diffuse_delta(gamma_artifact[:,time_range], channel_labels_pre, report_strings)
      mymetricsa[index,42], report_strings = process.detect_artifact.detect_diffuse_delta(f60hz_artifact [:,time_range], channel_labels_pre, report_strings)
      mymetricsa[index,43] = process.detect_artifact.detect_fractal_dimension(mysigs[:, time_range], channel_labels_pre, numsamples)

      mymetricsa[index,44], mymetricsa[index,45], mymetricsa[index,46] = process.detect_artifact.detect_drowsiness(alpha_artifact[:,time_range], channel_labels_pre)
      mymetricsa[index,47], mymetricsa[index,48], mymetricsa[index,49] = process.detect_artifact.detect_moments(beta_artifact[:,time_range], channel_labels_pre)
      mymetricsa[index,50], mymetricsa[index,51], mymetricsa[index,52] = process.detect_artifact.detect_moments(theta_artifact[:,time_range], channel_labels_pre)
      mymetricsa[index,53], mymetricsa[index,54], mymetricsa[index,55] = process.detect_artifact.detect_moments(delta_artifact[:,time_range], channel_labels_pre)
...
#   RUN ALL THE TESTS FOR THE DETECTION AND PHENOTYPES
    name_strings = ['STD Raw', 'Global STD', "PDR Symmetry", "PDR Synchrony", "PDR Regulation", "PDR Magnitude", "PDR Sinusoidal", "PDR Max Post.", "PDR FFT Width", 'PDR Max Amplitude', 'PDR Burst Width',
                    "Beta Max Front", "Front Alpha Asym", "XS Temp. Alpha", "Alpha Speed", 'Alpha Peak', 'Midline Beta', 'Focal Delta Index', 'Focal Delta Amp.',
    ...
    mymetrics[22], mymetrics[23], report_strings = process.detect_artifact.detect_phen_fastalpha(loalpha_artifact, alpha_artifact, hialpha_artifact, channel_labels_pre, report_strings, mysigs, numpages)
    mymetrics[24], report_strings = process.detect_artifact.detect_phen_spinbeta(beta_artifact, channel_labels_pre, report_strings)

    mymetrics[25], mymetrics[26], report_strings = process.detect_artifact.detect_focal_delta(delta_artifact, channel_labels_pre, report_strings, sites_rsi)
    mymetrics[27], mymetrics[28], report_strings = process.detect_artifact.detect_focal_theta(theta_artifact, channel_labels_pre, report_strings, sites_rsi)
    mymetrics[29], mymetrics[30], report_strings = process.detect_artifact.detect_focal_hibeta(hibeta_artifact, channel_labels_pre, report_strings, sites_rsi)
    mymetrics[31], mymetrics[32], report_strings = process.detect_artifact.detect_focal_hibeta(beta_artifact, channel_labels_pre, report_strings, sites_rsi) #FOCAL BETA
    mymetrics[33], report_strings = process.detect_artifact.detect_frontal_delta(delta_artifact, channel_labels_pre, report_strings)
    mymetrics[34], report_strings = process.detect_artifact.detect_phen_xsfrontslow(theta_artifact, channel_labels_pre, report_strings) #FRONTAL THETA
    mymetrics[35], report_strings = process.detect_artifact.detect_frontal_delta(gamma_artifact, channel_labels_pre, report_strings) #FRONTAL GAMMA
    mymetrics[36], report_strings = process.detect_artifact.detect_phen_frontasym(gamma_artifact, channel_labels_pre, report_strings) #FRONTAL GAMMA ASYM
    
    mymetrics[37], report_strings = process.detect_artifact.detect_diffuse_delta(delta_artifact, channel_labels_pre, report_strings)
    mymetrics[38], report_strings = process.detect_artifact.detect_diffuse_theta(theta_artifact, channel_labels_pre, report_strings)
    mymetrics[39], report_strings = process.detect_artifact.detect_diffuse_hibeta(hibeta_artifact, channel_labels_pre, report_strings)
    mymetrics[40], report_strings = process.detect_artifact.detect_diffuse_hibeta(beta_artifact, channel_labels_pre, report_strings) #DIFFUSE BETA
    mymetrics[41], report_strings = process.detect_artifact.detect_diffuse_delta(gamma_artifact, channel_labels_pre, report_strings)
    mymetrics[42], report_strings = process.detect_artifact.detect_diffuse_delta(f60hz_artifact, channel_labels_pre, report_strings)
    mymetrics[43] = process.detect_artifact.detect_fractal_dimension(mysigs, channel_labels_pre, numsamples)
    
    mymetrics[44], mymetrics[45], mymetrics[46] = process.detect_artifact.detect_drowsiness(alpha_artifact, channel_labels_pre)
    mymetrics[47], mymetrics[48], mymetrics[49] = process.detect_artifact.detect_moments(beta_artifact, channel_labels_pre)
    mymetrics[50], mymetrics[51], mymetrics[52] = process.detect_artifact.detect_moments(theta_artifact, channel_labels_pre)
    mymetrics[53], mymetrics[54], mymetrics[55] = process.detect_artifact.detect_moments(delta_artifact, channel_labels_pre)
    ...
    # added this  December 2024 to bail out when in server mode
    #exit()
