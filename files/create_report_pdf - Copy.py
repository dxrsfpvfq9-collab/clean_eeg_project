...
def create_report_pdf(name, short_name, channel_labels_short, name_strings, range_strings, report_strings, metrics, montage, excel_file_path, n, numpages, myvisualsigs, electrode_names):
    ...
    pdf_file.drawString(385, 780, pdf_text)

    df = pd.read_excel(excel_file_path, sheet_name='Sheet1')
    num_files = df.shape[1] - 2
    pdf_text1 =  'Number of files: '+str(num_files)
    pdf_file.drawString(385, 768, pdf_text1)
...
#    paragraph += Paragraph("It is based on calculations using signal processing methods to extract metrics related to the abundance and timing of recognized rhythms and patterns", normal_style)
    ...
    paragraph.drawOn(pdf_file, 320, 730)
...
#    leave this  one out, it makes an extra page feed we don't want
#    pdf_file.showPage()
    high_sentences =  ['The power in the raw EEG is exhibited to be high in comparison to other individuals. this may be artifact, but is not a concern',
                      ...
                      'Frontal gamma asymmetry reflects the mood regualtion, a high value may suggest extreme approach/positive affect/risk-taking',
                      ...
                      'Diffuse hibeta reflects generalized overactivity (busy brain), a high value may be a concern (anxiety, headaches, allery, drug effrect)',
                      ...
                      '60hz artifact reflects the enviornmental interference a high value reflects a high level if interference',
                      ...
                      'Theta moment 2 reflects the time course of theta, a high value may be a concern (inattention, unfocused)is high',
...
#    commented out December 2024 for server version
    pdf_file.showPage()
...
#  GET MICROVOLTS VALUES FOR DIFFERENT WAVE-------------------------------------------------------------------------------------------------------------------------------------
  ...
  #    if selstring[7] == 1:
    ...
    pdf_file.drawString(200, 696, "STS-AI EEG Quality Review")
